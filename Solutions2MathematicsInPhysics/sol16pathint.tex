...
\begin{equation*}
\begin{split}
	{}&\int_{-\infty}^\infty \exp\left( -i X^T O^T S O X + i B O^T O X \right)\prod_{i=1}^N  d x_i \\
	={}& \sqrt{\frac{\pi^N}{\det(S)}} \exp\left( \frac i 4  B^T O^T O A^{-1} O^T O B \right)
\end{split}
\end{equation*}
...
\begin{equation*}
\begin{split}
	\delta I ={}& \int_{-\infty}^\infty \left[ \exp\left( -i (Y+\delta Y)^T S (Y+ \delta Y) + i D^T(Y + \delta Y) \right) \right.\\
	{}&\left. - \exp\left( -i Y^T S Y + i D^T Y \right) \right]\prod_i^N dy_i \\
...
\begin{equation*}
\begin{split}
	\delta I ={}& \int_{-\infty}^\infty \left[ \exp\left(-(Y+\delta Y)^T S (Y+\delta Y) + D^T(Y+\delta Y)\right)\right. \\
	{}&\left. -\exp\left( -Y^T S Y + D^T Y \right)  \right]\prod_i^N dy_i\\
